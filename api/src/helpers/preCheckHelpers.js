...
export default class PreCheckHelpers {
  ...
  static async isAuthenPlaces(auth, placesId) {
    ...
    if (userInfo) {
      switch (Number(userInfo.userGroupsId)) {
  ...
  static async getInfoUser(auth) {
    ...
    const infoUser = await MODELS.findOne(users, {
      ...
      include: [
        { model: userGroups, as: 'userGroups', attributes: ['id', 'userGroupsName'], required: true },
        {
          model: places,
          as: 'places',
          attributes: ['id', 'placesName'],
          required: false,
          where: { status: 1 }
        }
      ],
