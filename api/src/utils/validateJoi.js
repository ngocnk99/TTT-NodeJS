...
export default class ValidateJoi {
  ...
  static assignSchema(schema, updates) {
    ...
    _.forIn(updates, (v, k) => {
      copySchema[k] = Object.keys(v).reduce((schemaProp, opsK) => {
  ...
  static createSchemaProp(options) {
    ...
    return Object.keys(options).reduce((r, k) => (options[k] === noArguments ? r[k]() : r[k](options[k])), Joi);
  }
  ...
  static ArticleGetByCategoriesParentToArray(obj, key) {
    ...
    if (arr.length === 1) {
      obj[key] = {
        $or: sequelize.literal(
          'EXISTS (select id from categories as t where (t.parentId in (' +
            obj[key] +
            ') or t.id in (' +
            obj[key] +
            ')) and t.id =articles.categoriesId)'
        )
      };
    } else {
      obj[key] = arr.map(n => n);
  ...
  static TemplateGetByTemplateGroupParentToArray(obj, key) {
    ...
    if (arr.length === 1) {
      obj[key] = {
        $or: sequelize.literal(
          'EXISTS (select id from templateGroups as t where (t.parentId in (' +
            obj[key] +
            ') or t.id in (' +
            obj[key] +
            ')) and t.id =templates.templateGroupsId)'
        )
      };
    } else {
      obj[key] = arr.map(n => n);
  ...
  static ProductGetByCategoriesParentToArray(obj, key) {
    ...
    obj[key] = {
      $or: sequelize.literal(
        'EXISTS (select id from categories as t where t.typesId=2 and (t.parent_Id in (' +
          obj[key] +
          ') or t.id in (' +
          obj[key] +
          ')) and t.id =ecommerceProducts.categoriesId)'
      )
    };
    console.log('obj===', obj);
