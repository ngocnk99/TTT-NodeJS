...
DOCKER_IMAGE=docker.vgasoft.vn/webportal-api-"$BUILD_TAG":"$BUILD_NUMBER"
ENV_FILE=/data/env/webportalapi/.env.production # Default to dev environment
CONTAINER_NAME=webportal-api-"$BUILD_TAG"

if [ "$BUILD_TAG" = "main" ]; then
  ENV_FILE=/data/env/webportalapi/.env.production # Production environment file
elif [ "$BUILD_TAG" = "test" ]; then
  ENV_FILE=/data/env/webportalapi/.env.production # Staging environment file
fi
